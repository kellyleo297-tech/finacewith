...
import { recorderAgent } from './agents/recorder';
import { analyzerAgent } from './agents/analyzer';
import { budgeterAgent } from './agents/budgeter';
import { saverAgent } from './agents/saver';
import { educatorAgent } from './agents/educator';
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // Step 1: Superviser Agent — classify intent
    const { intent } = await supervisorAgent(message);
    console.log(`[Supervisor] Intent: ${intent} | Message: "${message.slice(0, 50)}"`);
    ...
    switch (intent) {
      ...
      case 'finance_edu':
        ...
        break;
      case 'decision': {
        // Decision = analyze budget impact
        const analyzeResp = await analyzerAgent(
          `用户想买一个东西，问题：${message}。请分析这笔消费是否在预算范围内。`,
          ctx
        );
    ...
    // Step 3: Compliance check (only for finance_edu and saving intents)
    if (intent === 'finance_edu' || intent === 'saving') {
      const check = await complianceAgent(response.answer);
      console.log(`[Compliance] Status: ${check.status} | Issues: ${check.issues.length}`);
...
app.listen(PORT, () => {
  console.log(`🚀 MoneyMate Agent Server running on http://localhost:${PORT}`);
